# crag Anchor — packaging & install quickstart

crag Anchor is the **verified-memory engine** that sits under Claude Code /
...
`GET /health` with `{"ok": true, ...}`, and the `crag-anchor-mcp` entry point
...
> **Zero-config defaults are repo-relative.** With no `CRAG_ANCHOR_*` env and
...
- DB, logs, and the embedding model cache persist on the `crag-anchor-data`
  volume (`CRAG_ANCHOR_HOME=/data` inside the container).
...
crag-anchor                          # foreground; Ctrl-C to stop
# in another shell:
curl -sf http://127.0.0.1:8786/health
```
...
|---|---|
| `crag-anchor` | the FastAPI memory daemon (`apps/daemon/engine_daemon.py`) |
| `crag-anchor-mcp` | the stdio MCP server, a thin client of the daemon (`apps/mcp/mcp-server.py`) |
| `crag-anchor-cli` | operator lifecycle tooling — migrate/backfill/decay (`db/engine-cli.py`) |

Run at boot:

- **Linux** — `packaging/systemd/crag-anchor.service` (edit the User /
  WorkingDirectory / ExecStart paths, then `systemctl enable --now`).
- **macOS** — `packaging/launchd/sh.crag.anchor.plist` (edit the `CHANGE_ME`
  paths, then `launchctl load`).
- **Windows** — register a Task Scheduler task that runs the venv's
  `crag-anchor.exe` console script at logon (or use the Docker path).

## C. Just the MCP client (agent already has a daemon)

If a daemon is already running (locally or reachable via
`CRAG_ANCHOR_DAEMON_URL`), you only need the MCP entry point registered — see
the next section.
...
The MCP server resolves the daemon URL from `CRAG_ANCHOR_DAEMON_URL`, else
`CRAG_ANCHOR_DAEMON_HOST`/`CRAG_ANCHOR_DAEMON_PORT` via `db/stack.toml`, else
...
`CRAG_ANCHOR_GROUNDING_PROVIDER=anthropic-oauth` (the default). The daemon
...
export CRAG_ANCHOR_GROUNDING_PROVIDER=anthropic-api
...
`pip install -e '.[openai]'` + `CRAG_ANCHOR_GROUNDING_PROVIDER=openai` +
`CRAG_ANCHOR_LLM_BASE_URL=...`. All grounding LLM knobs live in `db/stack.toml`
`[grounding.llm]` / env (`CRAG_ANCHOR_GROUNDING_*`); see `db/grounding_config.py`.
...
|---|---|---|---|
| Engine root | `CRAG_ANCHOR_HOME` | `[paths].home` | repo root |
| DB file | `CRAG_ANCHOR_DB_PATH` | `[paths].db_path` | `<home>/db/engine.db` |
| Log dir | `CRAG_ANCHOR_LOG_DIR` | `[paths].log_dir` | `<home>/logs` |
| Daemon bind | `CRAG_ANCHOR_DAEMON_HOST` / `CRAG_ANCHOR_DAEMON_PORT` | `[paths].daemon_host` / `daemon_port` | `127.0.0.1:8786` |
| MCP → daemon | `CRAG_ANCHOR_DAEMON_URL` | (derived from bind) | `http://127.0.0.1:8786` |
| External sources | `CRAG_ANCHOR_SOURCE_<KEY>` | `[sources].<key>` | unset (fail-soft) |
| Grounding LLM | `CRAG_ANCHOR_GROUNDING_*` | `[grounding.llm]` | anthropic-oauth / haiku |
